...
parser.add_argument('--land', type=int, required=True, help = 'land point frame number')

args = parser.parse_args()

videofile_basename = os.path.splitext(os.path.basename(args.video))[0] # 去除副檔名
# 目的資料夾
directory = os.path.join(os.path.dirname(args.video), 'output')
...
while(True):
    ...
    if currentFrame not in visibility or visibility[currentFrame] == 0:
        ...
        if currentFrame == landPoint_frame:
            cv2.circle(image,(draw_x, draw_y), 8, (0,255,0), -1)
            cv2.putText(image, f"{frame[currentFrame]}", (draw_x, draw_y+50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 1, cv2.LINE_AA)
        else:

...
def setupOfflineTrackingNodes(project_name, cfg, replay_path, weights, page=None):
    ...
    for node_name, node_info in cfg.items():
        ...
        if 'node_type' in node_info:
            if node_info['node_type'] == 'Reader':
                camera_cfg = f"{ROOTDIR}/Reader/{node_info['brand']}/config/{node_info['hw_id']}.cfg"
...
class CameraReader(Node):
    def __init__(self, node_name, node_info):
        ...
        self.command = f"python3 {ROOTDIR}/Reader/{node_info['brand']}/main.py " f"--nodename {node_name}"
...
class TrackNet(Node):
    def __init__(self, project_name, node_name, node_info, load_path, weights, page):
        ...
        self.command = f"python3 {ROOTDIR}/TrackNet/TrackNet10/TrackNet.py " f"--nodename {node_name} --weights {weights} --data {video_path} --save_csv {load_path} --page {page}"

class TrackNet_offline(Node):
    def __init__(self, node_name, data, save_csv, weights):
        super().__init__()
        self.name = node_name
        self.command = f"python3 {ROOTDIR}/TrackNet/TrackNet10/TrackNet.py " f"--nodename {node_name} --weights {weights} --data {data} --save_csv {save_csv}"
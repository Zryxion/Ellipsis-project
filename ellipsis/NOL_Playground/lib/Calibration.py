...
class Calibration:
    ...
    def readVideo(self) -> None:
        """Convert video frame to images"""
        videos = glob.glob(os.path.join(self.video_folder, "chessboard_*.mp4"))
        ...
        for file in videos:
            ...
            img_folder = os.path.join(self.video_folder, f"frames_{basename}")
    ...
    def readImages(self):
        """Read images"""
        img_folders = glob.glob(os.path.join(self.video_folder, "frames_*/"))
        ...
        progressbar = tqdm.tqdm(total=len(img_paths), desc='Finding chessboard')

        core_count = os.cpu_count()

        img_paths = np.array_split(np.array(img_paths), core_count)
    ...
    def calibrateCamera(self):
        ...
        dataset = ImageDataset(self, self.readImages(), self.resolution)
        #dataset.filterByCustomRule()
        print(f"dataset length:{len(dataset.data)}")
        #data_t = dataset.popByCustomRule(50)
        data_t = list(dataset.popByMotivation())
        print(f"calibrate length:{len(data_t)}")
...
class ImageDataset:
    ...
    def popByMotivation(self):
        ...
        for i in range(1, len(self.data)):
            ...
            if prev_c is None or curr_c is None:
                continue
            if np.linalg.norm(self.data[i].corners[0][0] - self.data[i].corners[1][0]) < 40:
                continue
            if np.linalg.norm(self.data[i].corners[0][0] - self.data[i].corners[self.calib.corner][0]) < 40:
                continue

            if np.linalg.norm(prev_c-curr_c) < 5:
                selected.append(self.data[i])
        print(f"len:{len(selected)}")
    ...
    def filterByCustomRule(self):
        def filter_condition(img_data:ImageData):
            if img_data.corners is None:
                return False
            if np.linalg.norm(img_data.corners[0][0] - img_data.corners[1][0]) < 40:
                return False
            if np.linalg.norm(img_data.corners[0][0] - img_data.corners[self.calib.corner][0]) < 40:
                return False
            if img_data.blurScore > 150:
                return False
            return True
        self.data = list(filter(filter_condition, self.data))
    def popByCustomRule(self, n):
        set1 = self.popByGrid(self._chooseByArea(30, 40), round(n/3))
        set2 = self.popByGrid(self._chooseByArea(40, 60), round(n/3))
        set3 = self.popByGrid(self._chooseByArea(60, 70), round(n/3))
        return set1 + set2 + set3
...
class ImageData:
    ...
    def _findChessboardCorners(self, img):
        ...
        # 先降低解析度，運算得比較快
        #scaled_resolution = (round(self.calib.resolution[0]/SCALE_RATIO), round(self.calib.resolution[1]/SCALE_RATIO))
        #scaled_gray = cv2.resize(gray, scaled_resolution, interpolation=cv2.INTER_AREA)

        ret, corners = cv2.findChessboardCorners(gray, (self.calib.corner, self.calib.corner))
